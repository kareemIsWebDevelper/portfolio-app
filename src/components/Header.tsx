      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          ...
          <div className="hidden md:flex items-center space-x-2">
            <ThemeToggle />
            {[
              { icon: Github, color: 'hover:bg-slate-100 dark:hover:bg-slate-800' },
              { icon: Linkedin, color: 'hover:bg-blue-50 dark:hover:bg-blue-900/30' },
              { icon: Mail, color: 'hover:bg-green-50 dark:hover:bg-green-900/30' }
            ].map(({ icon: Icon, color }, index) => (
              <a
                key={index}
                href="#"
                className={`p-2 rounded-lg transition-all duration-300 transform hover:scale-110 hover:rotate-3 ${color}`}
                style={{ animationDelay: `${(index + 5) * 100}ms` }}
              >
                <Icon className="h-5 w-5 text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white transition-colors duration-300" />
              </a>
            ))}
          </div>
